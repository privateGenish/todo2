...
async function writeUser(req, res, next) {
  try {
    ...
    const decodedToken = await admin.auth().verifyIdToken(authorization);
    if (decodedToken.exp <= Math.floor(new Date().getTime() / 1000) && decodedToken.uid != uid) throw Error("Access Denied");
    return next();
  } catch (e) {
    console.log(e);
    if (e.stack.startsWith("TypeError") || e.stack.startsWith("ReferenceError") || e.message == "AssertionError")
      return res.status(400).send({ error: "Bad request" });
    if (e.message == "Access denied" || e.codePrefix == "auth") return res.status(401).send({ error: "Access denied" });
    return res.status(500).send({ error: "Internal Server Error" });
  }
}
