...
async function getUser(req, res) {
  try {
    ...
    return res.status(500).send({ error: "Internal Server Error", type: "internal error" });
  }
}
...
async function register(req, res) {
  try {
    ...
    return res.status(201).send({ message: `User has created successfully`, User: { uid: uid, name: name, email, email } });
  } catch (e) {
    if (e.code == "ConditionalCheckFailedException") return res.status(405).send({ error: "uid is already exists" });
...
async function deleteUser(req, res) {
  try {
    ...
    return res.status(200).send({ message: "User has deleted successfully", User: { uid: uid } });
  } catch (e) {
    console.log(e);
    return res.status(500).send({ error: "Internal Server Error" });
  }
